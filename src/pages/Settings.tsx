...
const Settings = () => {
  ...
  return (
    <div className="min-h-full bg-background pb-20 no-select">
      ...
      <main className="px-5">
        ...
        <Card>
          <Row
            icon={Palette}
            label="Customization"
            hint="Color theme, font, layout density & moods"
            right={
              <div className="flex items-center gap-2">
                <div
                  className="h-5 w-5 rounded-md border border-border"
                  style={{ backgroundColor: COLOR_THEMES.find((t) => t.id === colorThemeId)?.swatch }}
                />
                <span className="text-muted-foreground text-[18px] leading-none">›</span>
              </div>
            }
            onClick={() => { tapHaptic(); navigate("/settings/customization"); }}
            last
          />
        </Card>
      ...
      {/* Archived routines list */}
      <Dialog open={archivedOpen} onOpenChange={setArchivedOpen}>
        <DialogContent className="rounded-[28px] p-0 gap-0 max-w-[92vw] sm:max-w-md overflow-hidden">
          <DialogHeader className="px-6 pt-6 pb-4 border-b border-border">
            <DialogTitle className="text-xl font-serif font-bold text-left">Archived routines</DialogTitle>
            <DialogDescription className="text-left text-[13px]">
              Tap a routine to preview or unarchive it.
            </DialogDescription>
          </DialogHeader>
          <div className="max-h-[60vh] overflow-y-auto px-4 py-3 space-y-1.5">
            {archivedRoutines.length === 0 ? (
              ...
              archivedRoutines.map((rt) => (
                <button
                  key={rt.id}
                  onClick={() => {
                    tapHaptic();
                    setArchivedPreview(rt);
                  }}
      ...
      {/* Archived routine preview */}
      <Dialog open={!!archivedPreview} onOpenChange={(o) => !o && setArchivedPreview(null)}>
        <DialogContent className="rounded-[28px] p-0 gap-0 max-w-[92vw] sm:max-w-md overflow-hidden">
          {archivedPreview && (
            <>
              <DialogHeader className="px-6 pt-6 pb-4 space-y-2 border-b border-border">
                <div className="flex items-center gap-3">
                  <div className="h-12 w-12 shrink-0 rounded-xl bg-accent/5 border border-accent/10 flex items-center justify-center text-2xl">
                    {archivedPreview.emoji || "✨"}
                  </div>
                  <div className="flex-1 min-w-0 text-left">
                    <DialogTitle className="text-xl font-serif font-bold leading-tight">
                      {archivedPreview.title}
                    </DialogTitle>
                    {archivedPreview.description && (
                      <DialogDescription className="text-[12px] text-muted-foreground mt-0.5">
                        {archivedPreview.description}
                      </DialogDescription>
                    )}
              ...
              <div className="max-h-[55vh] overflow-y-auto px-6 py-4">
                <BlockPreview blocks={archivedPreview.blocks ?? []} />
              </div>
              <DialogFooter className="flex flex-row gap-3 sm:justify-end px-6 py-4 border-t border-border bg-muted/20">
                <Button
                  variant="outline"
                  onClick={() => setArchivedPreview(null)}
                  className="flex-1 rounded-2xl h-11 font-semibold border-border/60"
                >
                  Cancel
                </Button>
                <Button
                  onClick={() => {
                    r.updateRoutine(archivedPreview.id, { archived: false });
                    successHaptic();
